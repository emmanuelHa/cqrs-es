...
public class AccountAggregate extends AggregateRoot {
    ...
    public void withdrawFunds(double amount) {
        if(!this.active) {
            throw new IllegalStateException("Funds cannot be withdrawned from a closed account");
        }
        if(amount > this.balance) {
            throw new IllegalStateException(MessageFormat.format("The amount withdrawned shall not exceed the balance " +
                    "of your account {0}", this.balance));
        }
